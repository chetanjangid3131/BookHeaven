...
# 9. Admin Book Update Endpoint Check (JWT Auth)
from users.models import User
from rest_framework_simplejwt.tokens import RefreshToken

admin_user, _ = User.objects.get_or_create(email='admin_test@bookhaven.com', defaults={'is_staff': True, 'is_superuser': True})
...
res = client.patch(
    '/api/admin/books/1/',
    data={'title': 'Sapiens: Test Update'},
    content_type='application/json',
    HTTP_AUTHORIZATION=f'Bearer {token}'
)
print(f"PATCH /api/admin/books/1/ -> status: {res.status_code}, updated title: {res.json().get('title') if res.status_code==200 else res.content}")
assert res.status_code == 200, f"Expected 200, got {res.status_code}"

# 10. Verify GET /api/books/1/ reflects updated title
res = client.get('/api/books/1/')
print(f"GET /api/books/1/ after update -> title: {res.json().get('title')}")
assert res.json().get('title') == 'Sapiens: Test Update'
